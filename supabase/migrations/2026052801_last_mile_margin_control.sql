...
-- Creates the enterprise margin-control domain without touching wallet/payments.
-- =============================================================================
...
'Business-scope helper for Last-Mile RLS. Action permissions are enforced by server API routes.';
...
CREATE INDEX IF NOT EXISTS idx_last_mile_carriers_provider_key
    ON public.last_mile_carriers(business_id, provider_key);
CREATE INDEX IF NOT EXISTS idx_last_mile_carriers_profile
    ON public.last_mile_carriers(business_id, profile_user_id)
    WHERE profile_user_id IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_last_mile_carriers_fleet_member
    ON public.last_mile_carriers(business_id, fleet_member_id)
    WHERE fleet_member_id IS NOT NULL;
DROP TRIGGER IF EXISTS trg_last_mile_carriers_updated_at ON public.last_mile_carriers;
CREATE TRIGGER trg_last_mile_carriers_updated_at
    BEFORE UPDATE ON public.last_mile_carriers
    FOR EACH ROW
...
CREATE TABLE IF NOT EXISTS public.last_mile_route_lanes (
    ...
    service_level TEXT NOT NULL DEFAULT 'standard'
        CHECK (service_level IN ('standard', 'express', 'refrigerated', 'fragile', 'custom')),
...
CREATE INDEX IF NOT EXISTS idx_last_mile_lanes_business_status
    ON public.last_mile_route_lanes(business_id, status, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_last_mile_lanes_city_pair
    ON public.last_mile_route_lanes(business_id, origin_department, origin_city, destination_department, destination_city);
DROP TRIGGER IF EXISTS trg_last_mile_lanes_updated_at ON public.last_mile_route_lanes;
CREATE TRIGGER trg_last_mile_lanes_updated_at
    BEFORE UPDATE ON public.last_mile_route_lanes
    FOR EACH ROW
    EXECUTE FUNCTION public.set_updated_at_timestamp();
...
CREATE TABLE IF NOT EXISTS public.last_mile_contracts (
    ...
    source_kind TEXT NOT NULL DEFAULT 'manual'
        CHECK (source_kind IN ('manual', 'marketplace_observed', 'private_fleet_policy', 'renegotiated')),
    status TEXT NOT NULL DEFAULT 'draft'
        CHECK (status IN ('draft', 'active', 'paused', 'expired', 'superseded')),
    pricing_model TEXT NOT NULL DEFAULT 'per_trip'
        CHECK (pricing_model IN ('per_trip', 'per_km', 'per_kg', 'hybrid', 'monthly_retainer')),
    ...
    evidence_required JSONB NOT NULL DEFAULT jsonb_build_object(
        'pickup_pin', true,
        'delivery_pin', true,
        'delivery_photo', true,
...
CREATE INDEX IF NOT EXISTS idx_last_mile_contracts_business_status
    ON public.last_mile_contracts(business_id, status, starts_at DESC);
CREATE INDEX IF NOT EXISTS idx_last_mile_contracts_carrier_lane
    ON public.last_mile_contracts(business_id, carrier_id, lane_id, status);
CREATE INDEX IF NOT EXISTS idx_last_mile_contracts_expiry
    ON public.last_mile_contracts(business_id, ends_at)
    WHERE ends_at IS NOT NULL;
DROP TRIGGER IF EXISTS trg_last_mile_contracts_updated_at ON public.last_mile_contracts;
CREATE TRIGGER trg_last_mile_contracts_updated_at
    BEFORE UPDATE ON public.last_mile_contracts
    FOR EACH ROW
    EXECUTE FUNCTION public.set_updated_at_timestamp();
...
CREATE TABLE IF NOT EXISTS public.last_mile_contract_events (
    ...
    event_type TEXT NOT NULL
        CHECK (event_type IN ('created', 'activated', 'rate_changed', 'renegotiation_requested', 'renegotiated', 'paused', 'expired', 'manual_note')),
...
CREATE INDEX IF NOT EXISTS idx_last_mile_contract_events_business
    ON public.last_mile_contract_events(business_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_last_mile_contract_events_contract
    ON public.last_mile_contract_events(contract_id, created_at DESC);
...
CREATE TABLE IF NOT EXISTS public.last_mile_trip_cost_observations (
    ...
    source_kind TEXT NOT NULL DEFAULT 'sync'
        CHECK (source_kind IN ('cargo_offer', 'payment', 'private_fleet_allocation', 'manual_adjustment', 'sync')),
    execution_status TEXT NOT NULL DEFAULT 'planned'
        CHECK (execution_status IN ('planned', 'assigned', 'in_progress', 'completed', 'cancelled', 'disputed')),
    currency_code TEXT NOT NULL DEFAULT 'COP' CHECK (currency_code IN ('COP', 'USD', 'PEN', 'BRL')),
...
CREATE INDEX IF NOT EXISTS idx_last_mile_observations_business_period
    ON public.last_mile_trip_cost_observations(business_id, observed_at DESC);
CREATE INDEX IF NOT EXISTS idx_last_mile_observations_offer
    ON public.last_mile_trip_cost_observations(business_id, offer_id);
CREATE INDEX IF NOT EXISTS idx_last_mile_observations_carrier
    ON public.last_mile_trip_cost_observations(business_id, carrier_id, observed_at DESC);
CREATE INDEX IF NOT EXISTS idx_last_mile_observations_lane
    ON public.last_mile_trip_cost_observations(business_id, lane_id, observed_at DESC);
CREATE INDEX IF NOT EXISTS idx_last_mile_observations_contract
    ON public.last_mile_trip_cost_observations(contract_id, observed_at DESC);
DROP TRIGGER IF EXISTS trg_last_mile_observations_updated_at ON public.last_mile_trip_cost_observations;
CREATE TRIGGER trg_last_mile_observations_updated_at
    BEFORE UPDATE ON public.last_mile_trip_cost_observations
    FOR EACH ROW
    EXECUTE FUNCTION public.set_updated_at_timestamp();
...
CREATE INDEX IF NOT EXISTS idx_last_mile_scorecards_business_period
    ON public.last_mile_provider_score_snapshots(business_id, period_start DESC, period_end DESC);
CREATE INDEX IF NOT EXISTS idx_last_mile_scorecards_carrier
    ON public.last_mile_provider_score_snapshots(carrier_id, period_start DESC);
...
CREATE TABLE IF NOT EXISTS public.last_mile_renegotiation_recommendations (
    ...
    trigger_type TEXT NOT NULL
        CHECK (trigger_type IN ('cost_overrun', 'incident_rate', 'evidence_missing', 'volume_discount', 'supplier_underperformance', 'contract_expiring', 'benchmark_gap')),
    severity TEXT NOT NULL DEFAULT 'medium' CHECK (severity IN ('low', 'medium', 'high', 'critical')),
    status TEXT NOT NULL DEFAULT 'open'
        CHECK (status IN ('open', 'acknowledged', 'in_negotiation', 'accepted', 'rejected', 'closed')),
    title TEXT NOT NULL,
...
CREATE INDEX IF NOT EXISTS idx_last_mile_recommendations_business_status
    ON public.last_mile_renegotiation_recommendations(business_id, status, severity, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_last_mile_recommendations_carrier_period
    ON public.last_mile_renegotiation_recommendations(carrier_id, period_start DESC, period_end DESC);
CREATE INDEX IF NOT EXISTS idx_last_mile_recommendations_contract
    ON public.last_mile_renegotiation_recommendations(contract_id, status, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_last_mile_recommendations_dedupe
    ON public.last_mile_renegotiation_recommendations(business_id, dedupe_key);
DROP TRIGGER IF EXISTS trg_last_mile_recommendations_updated_at ON public.last_mile_renegotiation_recommendations;
CREATE TRIGGER trg_last_mile_recommendations_updated_at
    BEFORE UPDATE ON public.last_mile_renegotiation_recommendations
    FOR EACH ROW
    EXECUTE FUNCTION public.set_updated_at_timestamp();
...
CREATE TABLE IF NOT EXISTS public.last_mile_analysis_runs (
    ...
    run_type TEXT NOT NULL DEFAULT 'manual'
        CHECK (run_type IN ('manual', 'scheduled', 'offer_completed', 'backfill')),
...
CREATE INDEX IF NOT EXISTS idx_last_mile_runs_business_created
    ON public.last_mile_analysis_runs(business_id, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_last_mile_runs_status
    ON public.last_mile_analysis_runs(status, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_last_mile_runs_offer
    ON public.last_mile_analysis_runs(business_id, offer_id)
...
CREATE POLICY "Last mile carriers scoped by business"
    ON public.last_mile_carriers FOR ALL TO authenticated
...
CREATE POLICY "Last mile lanes scoped by business"
    ON public.last_mile_route_lanes FOR ALL TO authenticated
    USING (public.user_has_business_access(business_id))
    WITH CHECK (public.user_has_business_access(business_id));

DROP POLICY IF EXISTS "Last mile contracts scoped by business" ON public.last_mile_contracts;
CREATE POLICY "Last mile contracts scoped by business"
    ON public.last_mile_contracts FOR ALL TO authenticated
    USING (public.user_has_business_access(business_id))
    WITH CHECK (public.user_has_business_access(business_id));

DROP POLICY IF EXISTS "Last mile contract events scoped by business" ON public.last_mile_contract_events;
CREATE POLICY "Last mile contract events scoped by business"
    ON public.last_mile_contract_events FOR SELECT TO authenticated
    USING (public.user_has_business_access(business_id));

DROP POLICY IF EXISTS "Last mile observations scoped by business" ON public.last_mile_trip_cost_observations;
CREATE POLICY "Last mile observations scoped by business"
    ON public.last_mile_trip_cost_observations FOR ALL TO authenticated
    USING (public.user_has_business_access(business_id))
    WITH CHECK (public.user_has_business_access(business_id));

DROP POLICY IF EXISTS "Last mile scorecards scoped by business" ON public.last_mile_provider_score_snapshots;
CREATE POLICY "Last mile scorecards scoped by business"
    ON public.last_mile_provider_score_snapshots FOR ALL TO authenticated
    USING (public.user_has_business_access(business_id))
    WITH CHECK (public.user_has_business_access(business_id));

DROP POLICY IF EXISTS "Last mile recommendations scoped by business" ON public.last_mile_renegotiation_recommendations;
CREATE POLICY "Last mile recommendations scoped by business"
    ON public.last_mile_renegotiation_recommendations FOR ALL TO authenticated
    USING (public.user_has_business_access(business_id))
    WITH CHECK (public.user_has_business_access(business_id));

DROP POLICY IF EXISTS "Last mile runs scoped by business" ON public.last_mile_analysis_runs;
CREATE POLICY "Last mile runs scoped by business"
    ON public.last_mile_analysis_runs FOR SELECT TO authenticated
    USING (public.user_has_business_access(business_id));

GRANT SELECT, INSERT, UPDATE ON public.last_mile_carriers TO authenticated;
GRANT SELECT, INSERT, UPDATE ON public.last_mile_route_lanes TO authenticated;
GRANT SELECT, INSERT, UPDATE ON public.last_mile_contracts TO authenticated;
GRANT SELECT ON public.last_mile_contract_events TO authenticated;
GRANT SELECT, INSERT, UPDATE ON public.last_mile_trip_cost_observations TO authenticated;
GRANT SELECT, INSERT, UPDATE ON public.last_mile_provider_score_snapshots TO authenticated;
GRANT SELECT, INSERT, UPDATE ON public.last_mile_renegotiation_recommendations TO authenticated;
GRANT SELECT ON public.last_mile_analysis_runs TO authenticated;

UPDATE public.billing_plans
SET
    tagline = CASE code
        WHEN 'free' THEN 'Acceso operativo gratis para validar evidencia esencial y hasta 50 viajes/mes.'
        WHEN 'growth' THEN 'Operacion B2B inicial con evidencia, bodegas base, flota privada y 500 viajes/mes.'
        WHEN 'scale' THEN '3PL, API/webhooks, control tower, scorecards basicos y 2.000 viajes/mes.'
        WHEN 'enterprise' THEN 'Volumen personalizado, control de margen, contratos por proveedor, auditoria y soporte premium.'
        ELSE tagline
    END,
    price_monthly_cop = CASE code
        WHEN 'free' THEN 0
        WHEN 'growth' THEN 299000
        WHEN 'scale' THEN 799000
        WHEN 'enterprise' THEN 4500000
