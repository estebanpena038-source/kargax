/**
 * =============================================================================
 * KARGAX - MARKETPLACE POD REPORTS API
 * /lib/pod-marketplace/api.ts
 *
...
const logger = {
    log: (...args: unknown[]) => IS_DEV && console.log('[MarketplacePodAPI]', ...args),
    error: (...args: unknown[]) => console.error('[MarketplacePodAPI]', ...args),
    warn: (...args: unknown[]) => IS_DEV && console.warn('[MarketplacePodAPI]', ...args),
};
...
function mapOfferStatusToMarketplacePodStatus(status: string, pickupVerified: boolean, deliveryVerified: boolean): MarketplacePodStatus {
...
function determineMarketplacePodPhase(pickupVerified: boolean): MarketplacePodPhase {
    return pickupVerified ? 'delivery' : 'loading';
}

function mapItemStatus(item: DbManifestItem): MarketplacePodItemStatus {
...
function transformPickingEvents(events: DbPickingEvent[]): MarketplacePodTimelineEvent[] {
    return events.map(event => ({
        id: event.id,
        eventType: event.event_type as MarketplacePodEventType,
        ...
        itemStatus: event.item_status as MarketplacePodItemStatus | undefined,
...
function eventMatchesManifestItem(event: DbPickingEvent, item: MarketplacePodManifestItem): boolean {
...
function filterTimelineEventsForMarketplacePod(
...
function transformTripPhotos(photos: DbTripPhoto[] | null, offerId: string): MarketplacePodPhoto[] {
...
function calculateSummary(items: MarketplacePodManifestItem[], photos: MarketplacePodPhoto[]): MarketplacePodSummary {
...
function buildMarketplacePodVerifiedLocation(
...
): MarketplacePodVerifiedLocation | undefined {
...
export async function getMarketplacePodReport(offerId: string): Promise<GetMarketplacePodReportResponse> {
    logger.log('Fetching marketplace POD report for offer:', offerId);
    ...
    try {
        ...
        if (!offer) {
            const { data: privateOffer } = await supabase
                .from('cargo_offers')
                .select('id, is_private_fleet')
                .eq('id', offerId)
                .maybeSingle();

            if ((privateOffer as { is_private_fleet?: boolean } | null)?.is_private_fleet === true) {
                return {
                    success: false,
            ...
            return { success: false, error: 'No encontramos este expediente POD.', errorCode: 'NOT_FOUND' };
        }
        ...
        // Trucker info
        let truckerInfo: MarketplacePodTruckerInfo = { id: dbOffer.assigned_trucker_id || '', fullName: 'Camionero' };
        ...
        const timelineEvents = filterTimelineEventsForMarketplacePod(dbEvents, manifestItems);
        const timeline = transformPickingEvents(timelineEvents);

        // Also include photos from picking events in the gallery
        const eventPhotos: MarketplacePodPhoto[] = dbEvents
            ...
            .flatMap((e) =>
                (e.photo_urls || []).map((url, pIdx) => ({
                    ...
                    itemStatus: e.item_status as MarketplacePodItemStatus | undefined,
        ...
        const report: MarketplacePodReport = {
            offerId: dbOffer.id,
            status: mapOfferStatusToMarketplacePodStatus(dbOffer.status, pickupVerified, deliveryVerified),
            phase: determineMarketplacePodPhase(pickupVerified),
            trucker: truckerInfo,
            route: {
            ...
            originLocation: buildMarketplacePodVerifiedLocation(
            ...
            destinationLocation: buildMarketplacePodVerifiedLocation(
        ...
        logger.error('Unexpected error in getMarketplacePodReport:', error);
...
export async function getMarketplacePodList(
    businessId: string,
    options: { status?: MarketplacePodStatus; limit?: number; offset?: number } = {}
): Promise<{
    success: boolean;
    data?: Array<Pick<MarketplacePodReport, 'offerId' | 'status' | 'phase' | 'trucker' | 'route' | 'summary' | 'createdAt'>>;
    error?: string;
    total?: number;
}> {
    logger.log('Fetching marketplace POD list for business:', businessId, options);

    try {
        let query = supabase
            .from('cargo_offers')
            .select(`
                id, is_private_fleet, status, cargo_type, origin_department, origin_city, destination_department, destination_city,
        ...
        if (error) {
            logger.error('Error fetching marketplace POD list:', error);
            return { success: false, error: error.message };
        }

        const pods = (data || []).map(offer => {
            const dbOffer = offer as unknown as DbCargoOffer;
        ...
        return { success: true, data: pods, total: count || 0 };
    } catch (error) {
        logger.error('Unexpected error in getMarketplacePodList:', error);
...
export const marketplacePodApi = {
    getMarketplacePodReport,
    getMarketplacePodList,
};

export default marketplacePodApi;
