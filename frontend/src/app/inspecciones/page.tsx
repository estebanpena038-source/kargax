...
const statusLabels: Record<InspectionStatus | 'all' | 'issues', string> = {
    all: 'Todas',
    issues: 'Con novedad',
    pending: 'Pendientes',
...
function InspectionCard({ inspection, locale }: { inspection: InspectionListItem; locale: string }) {
    const hasIssues = inspection.summary.rejectedItems > 0;
    const route = `${inspection.route.originCity || 'Origen'} -> ${inspection.route.destinationCity || 'Destino'}`;

    return (
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
            <Link href={`/inspecciones/${inspection.offerId}`} className="block min-w-0">
                <Card className="group kx-trip-panel border-zinc-200 bg-white p-4 transition-all hover:border-zinc-950 hover:shadow-[0_24px_60px_-48px_rgba(10,10,10,.65)]">
                        ...
                        <div className="min-w-0">
                            ...
                            <div className="mt-2 flex flex-wrap items-center gap-3 text-sm text-zinc-600">
                                <span className="inline-flex items-center gap-1.5">
                                    ...
                                    {inspection.trucker.fullName || 'Conductor'}
                                </span>
                                <span className="font-money text-xs">{formatInspectionDate(inspection.createdAt, locale)}</span>
                            </div>
                        </div>
                    ...
                    <div className="kx-trip-metrics mt-4 grid gap-2">
                        {[
                            ['Items', inspection.summary.totalItems],
                            ['Cargados', inspection.summary.loadedItems],
                            ['Entregados', inspection.summary.deliveredItems],
                            ['Rechazados', inspection.summary.rejectedItems],
                            ['Fotos', inspection.summary.totalPhotos],
                        ].map(([label, value]) => (
...
function EmptyState() {
    return (
        <Card className="border-zinc-200 bg-white p-10 text-center">
            ...
            <h3 className="mt-5 text-lg font-semibold text-zinc-950">Aun no hay inspecciones registradas.</h3>
            <p className="mx-auto mt-2 max-w-md text-sm leading-6 text-zinc-600">
                Cuando un viaje tenga cargue, entrega o evidencia, aparecera aqui como expediente operativo.
            </p>
...
export default function InspectionsListPage() {
    ...
    const [inspections, setInspections] = React.useState<InspectionListItem[]>([]);
    const [isLoading, setIsLoading] = React.useState(true);
    const [error, setError] = React.useState<string | null>(null);
    const [searchQuery, setSearchQuery] = React.useState('');
    const [statusFilter, setStatusFilter] = React.useState<InspectionStatus | 'all' | 'issues'>('all');
    const locale = 'es-CO';
    ...
    React.useEffect(() => {
        async function fetchInspections() {
            ...
            setError(null);
            try {
                const response = await getInspectionList(user.id, { limit: 50 });
                if (response.success && response.data) {
                    setInspections(response.data as InspectionListItem[]);
                } else {
                    setError(response.error || 'No pudimos cargar inspecciones.');
                }
            } catch (err) {
                setError(err instanceof Error ? err.message : 'No pudimos cargar inspecciones.');
            } finally {
                setIsLoading(false);
            }
        }
    ...
    const metrics = React.useMemo(() => ({
        ...
        issues: inspections.filter((item) => item.summary.rejectedItems > 0).length,
        rejected: inspections.reduce((total, item) => total + item.summary.rejectedItems, 0),
        completed: inspections.filter((item) => item.status === 'completed').length,
        pending: inspections.filter((item) => item.status !== 'completed').length,
    }), [inspections]);
    ...
    const filteredInspections = React.useMemo(() => {
        ...
        return inspections.filter((inspection) => {
            const matchesQuery = !query
                || inspection.route.originCity?.toLowerCase().includes(query)
                || inspection.route.destinationCity?.toLowerCase().includes(query)
                || inspection.trucker.fullName?.toLowerCase().includes(query);
            const matchesStatus = statusFilter === 'all'
                || (statusFilter === 'issues' ? inspection.summary.rejectedItems > 0 : inspection.status === statusFilter);
        ...
        <DashboardLayout pageTitle="Inspecciones">
            <div className="kx-trip-container space-y-6 p-3 min-[380px]:p-4 md:p-6 lg:p-8">
                <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
                    <div>
                        <h1 className="text-2xl font-semibold tracking-tight text-zinc-950">Inspecciones</h1>
                        <p className="mt-1 text-sm text-zinc-600">Evidencia de cargue, ruta y entrega.</p>
                    </div>
                ...
                {error ? (
                    <Card className="border-zinc-300 bg-white p-4">
                        <div className="flex items-start gap-3 text-zinc-800">
                            ...
                            <div>
                                <p className="font-semibold">No pudimos cargar inspecciones.</p>
                ...
                {filteredInspections.length === 0 && !error ? (
                    ...
                    <div className="space-y-3">
                        {filteredInspections.map((inspection) => (
                            <InspectionCard key={inspection.offerId} inspection={inspection} locale={locale} />
                        ))}
                    </div>
                )}
            </div>
