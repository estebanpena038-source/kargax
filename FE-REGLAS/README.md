...
**Alcance:** `frontend/` Next.js 16 + React 19 + TypeScript + Tailwind CSS 4 + Radix + TanStack + Recharts + Playwright.  
**Uso:** obligatorio para cualquier vista nueva, refactor responsive, auditoría visual o revisión QA.

## Qué es FE-REGLAS

`FE-REGLAS/` es el sistema operativo de reglas frontend de KargaX. Define cómo debe verse, comportarse y probarse cada pantalla del SaaS logístico en celulares, tablets, laptops, desktop, monitores grandes y ultra wide.

No es una guía genérica de diseño. Es una especificación de ingeniería para que KargaX se sienta como un **sistema operativo logístico premium B2B**: rápido, claro, accesible, confiable y vendible a empresas grandes.
...
## Stack detectado y reglas base ya existentes

Antes de proponer dependencias nuevas, el stack real revisado en `frontend/package.json` ya incluye herramientas suficientes para un frontend premium:

- Framework: `next` 16.1.1, `react` 19.2.3, `react-dom` 19.2.3.
- Estilos: `tailwindcss` 4, `@tailwindcss/postcss`, `clsx`, `class-variance-authority`, `tailwind-merge`.
...
- Tipos y schemas: `frontend/src/types/**`, `zod` schemas o tipos locales.
...
- `src/app/api/billing/**`.
- Cualquier componente de wallet/liquidaciones.
...
## Fuentes técnicas usadas como base
...
- MDN `clamp()`, `min()`, `max()`: https://developer.mozilla.org/en-US/docs/Web/CSS/Reference/Values/clamp
