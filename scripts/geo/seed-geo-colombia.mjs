...
const DIVIPOLA_DATA_URL = process.env.GEO_DIVIPOLA_URL || 'https://www.datos.gov.co/resource/gdxc-w37w.json?$limit=50000';
...
function buildRows(rawRows) {
  ...
  for (const row of rawRows) {
    ...
    const departmentName = cleanName(pick(row, [
      'departamento', 'nombre_departamento', 'dpto', 'dpto_cnmbr', 'departamento_nombre', 'nom_departamento',
    ]));
    ...
    const municipalityName = cleanName(pick(row, [
      'municipio', 'nombre_municipio', 'nom_mpio', 'mpio_cnmbr', 'municipio_nombre', 'nom_municipio',
    ]));
