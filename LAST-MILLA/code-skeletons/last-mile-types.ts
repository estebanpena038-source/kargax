...
export type LastMileContractSource = 'manual' | 'marketplace_observed' | 'private_fleet_policy' | 'renegotiated';
...
export type LastMileExecutionStatus = 'planned' | 'assigned' | 'in_progress' | 'completed' | 'cancelled' | 'disputed';
export type LastMileRecommendationTrigger =
  | 'cost_overrun'
  | 'incident_rate'
  | 'evidence_missing'
  | 'volume_discount'
...
export interface LastMileContract {
  ...
  carrier_id: string;
  lane_id: string | null;
  source_kind: LastMileContractSource;
...
export interface LastMileTripCostObservation {
  id: string;
  business_id: string;
  offer_id: string;
  ...
  execution_status: LastMileExecutionStatus;
...
export interface LastMileRenegotiationRecommendation {
  ...
  period_start: string;
  period_end: string;
  trigger_type: LastMileRecommendationTrigger;
...
export interface LastMileAccessFlags {
  enabled: boolean;
  readOnly: boolean;
  canViewFinance: boolean;
  canManageContracts: boolean;
  canRunSync: boolean;
  canResolveRecommendations: boolean;
  canExport: boolean;
}

export interface LastMileSummaryResponse {
  period: { start: string; end: string };
  access: LastMileAccessFlags;
  metrics: {
    totalTrips: number;
  ...
  topRoutes: Array<{ laneId: string | null; label: string; trips: number; leakageCop: number; avgOverrunPct: number }>;
  topCarriers: Array<{ carrierId: string | null; name: string; type: string; trips: number; leakageCop: number; score: number }>;
  recommendations: LastMileRenegotiationRecommendation[];
}

export interface CreateLastMileContractPayload {
  carrierId: string;
  laneId?: string | null;
  sourceKind?: LastMileContractSource;
