...
| Wallet -> withdrawal | trucker, finance admin, admin plataforma | ledger marketplace consistente, hold/payout y reversal visibles sin incluir flota privada | transaccion, before/after, payout attempt, decision admin |
...
| Automatic payouts | trucker, finance admin, admin plataforma | retiro marketplace pasa por `cobre` o fallback manual sin doble pago | payout attempt, idempotency key, webhook/status |
| Private fleet external proof | business owner, finance_accountant, private_fleet_driver | flete/nomina privada quedan documentales y no aumentan wallet marketplace | payroll run, proof, wallet before/after, ledger privado |
...
| Entrega marketplace | trucker | viaje marketplace en progreso y pago completado | registrar llegada, evidencia y PIN entrega | settlement trigger marketplace correcto | `trip_deposit`, `money_rail=marketplace_freelancer`, offer completada | screenshot + ledger | pendiente |
| Entrega flota privada | private_fleet_driver | viaje privado en progreso | registrar llegada, evidencia y PIN entrega | cierre sin payout marketplace | `freight_payment=external_proof_pending`, sin `trip_deposit`, sin `payout_attempt` | screenshot + ledger | pendiente |
| Wallet | trucker | wallet existente | abrir `/billetera`, revisar timeline | `Marketplace freelancer` y `Flota privada` separados | wallet timeline consistente, private ledger no retirable | screenshot | pendiente |
| Retiro marketplace | trucker, finance admin | `marketplaceWallet.availableCop >= 50000` | solicitar retiro y decidirlo en admin | request y resolution validos | withdrawal / reversal / payout attempt visibles en ledger | screenshot + payload | pendiente |
...
| Retiro marketplace Nequi | trucker | saldo marketplace y metodo Nequi guardado | registrar Nequi y retirar | payout `manual_review` si automatic off, `queued|processing|paid|failed` si `cobre` activo | payout attempt con provider `manual` o `cobre` | screenshot + provider ref | pendiente |
| Retiro marketplace banco | trucker | saldo marketplace y cuenta guardada | registrar cuenta y retirar | payout `manual_review` si automatic off, `queued|processing|paid|failed` si `cobre` activo | payout attempt con provider `manual` o `cobre` | screenshot + provider ref | pendiente |
| Nomina privada comprobante externo | finance_accountant, private_fleet_driver | conductor privado con salario mensual | crear nomina, aprobar, subir comprobante, marcar `paid_external` | no se crea saldo wallet | `private_fleet_payment_proofs`, payroll `paid_external`, wallet before/after igual | screenshot + SQL | pendiente |
...
| Wallet -> withdrawal | `/billetera`, `/admin` | before/after de marketplace, transaccion, payout attempt, decision admin, reversal si aplica | ledger marketplace consistente y retiro visible |
...
| Automatic payouts | `/billetera`, `/admin`, provider sandbox | metodo retiro, payout attempt, webhook/status, fallback | saldo marketplace y provider conciliables |
| Private fleet external proof | `/dashboard/flota`, `/billetera`, `/dashboard/inteligencia` | payroll, comprobante, estado `paid_external`, wallet before/after | flota privada documental sin saldo retirable |
